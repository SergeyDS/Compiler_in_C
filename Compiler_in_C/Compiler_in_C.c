...
int main(int argc, char *argv[]){
   ...
   sprintf_s(output_filename, "%s.asm", argv[2]);
  ...
  // Node *test = parser(tokens);
   ...
   sprintf_s(nasm, "nasm -f elf64 generated.asm -o generated.o", argv[2], argv[2]);
   sprintf_s(gcc, "gcc generated.o -o %s -lc -no-pie", argv[2], argv[2]);
   system(nasm);
   system(gcc);
   printf("FINISHED\n");
}